// hex_to_7seg.v — 4-bit hex to 7-segment decoder
// Go Board 7-seg is active-low: 0 = segment ON
module hex_to_7seg (
    input  wire [3:0] i_hex,
    output reg  [6:0] o_seg   // {a, b, c, d, e, f, g}
);
    reg [6:0] seg_active;  // Active-high internally
    ...
    always @(*) begin
        ...
        endcase
        o_seg = ~seg_active;  // Invert for active-low hardware
    end
endmodule
