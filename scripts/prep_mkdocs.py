...
{# Giscus comments — per-page discussion threads backed by GitHub Discussions
   on ucf-draco-mike/hdl-for-dsd-student. The IDs below come from
   https://giscus.app after the Giscus app is installed on that repo and
   Discussions are enabled there. Until those IDs are filled in, the widget
   loads but reports a config error in the browser console (no impact on the
   rest of the page). See README "Course Site -> Per-page Feedback" for setup. #}
